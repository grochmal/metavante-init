 ...
 *
 * Purpose: Animation with SDL, second attempt.
 * Comments:
 *****************************************************************************/
...
/******************************************************************************
 * Function: init_penguins
 *
 * Purpose: This routine loops through the array of penguins
 *          and sets each to a random starting position
 *          and direction.
 * Input: penguins (static).
 *
 * Comments:
 *****************************************************************************/
static void init_penguins(void)
...
int main(void)
{
     ...
     * performance of SDL_BlitSurface, as it will not have
     * to convert the surface on the fly.
     */
    ...
    /*
     * Set penguin's alpha channel.  Ask for RLE acceleration,
     * a technique that can significantly speed up blending.
     */
    SDL_SetAlpha(temp, SDL_SRCALPHA | SDL_RLEACCEL, 0);

    /*
     * Convert the penguin to the display's format. We do this after
     * alpha processing as transparency can be optimized for some
     * particular displays.
     */
    /*viking_tux = SDL_DisplayFormat(temp);*/
    /*
     * Or we do not have alpha or no optimization.  But using colorkey
     * is too painful and the display is pixelated.
     */
    viking_tux = temp;
    if (NULL == viking_tux)
      {
    ...
    /* Animate FRAME_NUM frames (approximately FRAME_NUM / 30 seconds). */
    for (frames = 0; frames < FRAME_NUM; frames++)
      {
        ...
        /* Changing the delay you control the animation speed. */
        SDL_Delay(3);
      }
 ...
 *
 * Purpose: This routine moves each penguin by it's motion vector.
 * Input: penguins (static).
 *
 * Comments:
 *****************************************************************************/
 ...
 *
 * Purpose: This routine draws each penguin to the screen surface.
 * Input: viking_tux (static), penguins (static).
 * Output: screen (static).
 *
 * Comments:
 *****************************************************************************/
static void draw_penguins(void)
